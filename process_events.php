...
if (isset($_GET['action'])) {
    ...
    if ($action == 'approval' && $_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_POST['event_id'], $_POST['points'], $_POST['remarks'])) {
            ...
            if ($conn->query($sql_update) === TRUE) {
                ...
                //file_put_contents('log.txt', "SQL Error: " . $conn->error, FILE_APPEND);
                $_SESSION['error'] = "Error approving event: " . $conn->error;
            }

            // Redirect back to faculty dashboard
            header('Location: faculty_dashboard.php?section=activities');
            exit();
        } else {
    ...
    elseif ($action == 'delete' && isset($_GET['event_id']) && isset($_GET['file_path'])) {
        ...
        // Redirect back to faculty dashboard
        header('Location: faculty_dashboard.php?section=deletion');
        exit();
    }
    ...
    elseif ($action == 'edit' && $_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (isset($_POST['event_id'], $_POST['event_type'], $_POST['from_date'], $_POST['to_date'], $_POST['event_level'], $_POST['category'])) {
            $event_id = (int)$_POST['event_id'];
            ...
            $sql_edit = "UPDATE events 
                         SET event_type = '$event_type', from_date = '$from_date', to_date = '$to_date', event_level = '$event_level', category='$category'
                         WHERE id = '$event_id'";
            ...
            // Redirect back to faculty dashboard
            header('Location: faculty_dashboard.php?section=edition');
            exit();
        } else {
            $_SESSION['error'] = "Invalid input data.";
            header('Location: faculty_dashboard.php');
    ...
    //file_put_contents('log.txt', "Invalid POST data", FILE_APPEND);
